...
# This script detects the OS of the VM and then selects appropriate scripts to install the dependencies for MATLAB Web App Server on this VM.
# The script switches between yum and apt based on the OS detected
# Supported OS includes: (See MathWorks platform support for more details.)
# * rhel
# * centos
# * ubuntu
#
# This is a helper script called by startup script for the Google compute instance.
# This script does not run locally on the user's system.
# This script is accessed from a Google cloud storage bucket by the VM where MATLAB Web App Server and MATLAB Runtime is installed.
...
## Install MATLAB Production Server dependencies and gcsfuse
# Depending on OS flavour the script switches between apt and yum to fulfill dependencies

if [ ${arr[1]} = '"centos"' ]; then
  echo "MATLAB Production Server will be installed on OS: " ${arr[1]}
